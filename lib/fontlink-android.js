...
if (require.main === module) {
    (async () => {
        console.log('this is fontlink-android.ts');
        const resDir = process.argv[2];
        // create resDir / font / abcdiatype_italic.otf
        // create resDir / values / fontlink.xml
        // <resources>
        //   <string name="app_name">phiber</string>
        const fontFiles = [
            '/Users/mmoeller/workspace/straiqr/phiber-app/assets/fonts/PitchSans-Medium.otf',
        ];
        for (const fontFile of fontFiles) {
            console.log(fontFile, isFontFile(fontFile));
            console.log(getPostScriptName(await fs.promises.readFile(fontFile)));
        }
        await fs.promises.mkdir(path.join(resDir, 'font'), { recursive: true });
        await fs.promises.writeFile(path.join(resDir, 'font', 'pitchsans_medium.otf'), await fs.promises.readFile('/Users/mmoeller/workspace/straiqr/phiber-app/assets/fonts/PitchSans-Medium.otf'));
        await fs.promises.mkdir(path.join(resDir, 'values'), { recursive: true });
        await fs.promises.writeFile(path.join(resDir, 'values', 'fontlink.xml'), `<resources><string name="xxxtest">value</string></resources>\n`);
...
//# sourceMappingURL=fontlink-android.js.map