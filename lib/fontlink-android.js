...
if (require.main === module) {
    (async () => {
        ...
        // this is fontlink-android.ts
        // fontFile /Users/mmoeller/workspace/straiqr/phiber-app/assets/fonts/PitchSans-Medium.otf
        // isFontFile true
        // getPostScriptName PitchSans-Medium
        // resourceName pitchsans-medium
        // extname .otf
        if (fontlinkData.length > 0) {
            await fs.promises.writeFile(path.join(resDir, 'values', 'fontlink.xml'), `<resources><array name="fontlink">${fontlinkData.map((i) => `<item>${i}</item>`).join()}</array></resources>\n`);
        }
